...
<div class="container-fluid">
    ...
    <nav class="nav nav-pills mb-3">
        <a class="nav-link active" href="#" data-toggle="modal" data-target="#addCaseModal">Add case</a>
    </nav>

    <ul class="nav nav-tabs" id="myTab" role="tablist">
        <li class="nav-item">
            <a class="nav-link active" id="home-tab" data-toggle="tab" href="#home" role="tab" aria-controls="home" aria-selected="true">Active Cases</a>
        </li>
        <li class="nav-item">
            <a class="nav-link" id="profile-tab" data-toggle="tab" href="#profile" role="tab" aria-controls="profile" aria-selected="false">Inactive Cases</a>
        </li>

    </ul>
    <div class="tab-content" id="myTabContent">
        <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
           @if(count($legal_cases) > 0)
            <table class="table table-striped table-hover" id="cases-table">
                ...
                <tbody>
                @foreach($legal_cases as $lc)
                    <tr>
                        <td id="case-uuid">{{ $lc->case_uuid }}</td>
                        <td>{{ $lc->name }}</td>
                        <td>{{ $lc->status }}</td>
                        <td>{{ \Carbon\Carbon::parse($lc->open_date)->format('m/d/Y') }}</td>
                        <td>@mdo</td>
                    </tr>
                @endforeach
                </tbody>
            ...
            @else
                <div id="no-cases"><p>No cases, yet!  Add a case above</p></div>
            @endif
